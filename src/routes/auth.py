...
def init_oauth(app):
    ...
    # Stack Auth OIDC configuration (replaces Google)
    ...
    if not stack_client_id or not stack_client_secret or not stack_metadata_url:
        # Fallback to Google only if Stack is not configured; otherwise raise at runtime
...
def login():
    ...
    # Use configured provider (Stack if available, otherwise Google fallback name)
    ...
    if frontend_origin:
        ...
        # Use original host/proto from proxy headers (ProxyFix enabled in app)
        # request.host_url includes scheme://host/ with trailing slash
        base_origin = request.host_url.rstrip('/')
    redirect_uri = base_origin + url_for('auth.callback')
    return provider.authorize_redirect(redirect_uri, state=state)
